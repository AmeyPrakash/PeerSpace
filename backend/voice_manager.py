"""
PeerSpace WebRTC Signaling & Voice Matchmaking Manager
Handles peer pairing, SDP offer/answer relay, and ICE candidate exchange.
"""
...
logger = logging.getLogger("peerspace.voice")

class VoiceMatchmakingManager:
    def __init__(self):
    ...
    async def connect(self, ws: WebSocket, session_id: str, alias: str):
        ...
        if matched_partner:
            ...
            logger.info("Voice match created: Room %s between %s and %s", room_id, matched_partner['alias'], alias)
            ...
            if room:
                ...
                try:
                    await other["ws"].send_json({
                        ...
                        "message": "Your peer has disconnected from the call."
                    })
                except Exception:
                    pass

voice_manager = VoiceMatchmakingManager()
